...
struct AvifOptions {
  ...
  int bitDepth;
  // CICP (ITU-T H.273) color signal info written into the AVIF nclx box.
  // -1 = leave the codec defaults alone (matches upstream jsquash behavior).
  int cicpColorPrimaries;          // e.g. 12 = display-p3, 9 = bt2020
  int cicpTransferCharacteristics; // e.g. 13 = sRGB, 16 = SMPTE2084 (PQ), 18 = HLG
  int cicpMatrixCoefficients;      // e.g. 6 = bt601, 9 = bt2020-ncl, 0 = identity (RGB)
  // Whether the YUV samples cover the full code range (true) or the narrow
  // studio range (false). For HDR content limited (studio) range is the norm.
  bool fullRange;
};
...
val encode(std::string buffer, int width, int height, AvifOptions options) {
  ...
  // Apply caller-supplied CICP overrides. Each is opt-in: passing -1 keeps
  // the upstream default. The matrix override has to come after the lossless
  // branch above so callers can pick e.g. identity-RGB even at non-lossless
  // quality, or BT.2020-NCL for PQ HDR content.
  if (options.cicpColorPrimaries >= 0) {
    image->colorPrimaries = (avifColorPrimaries)options.cicpColorPrimaries;
  }
  if (options.cicpTransferCharacteristics >= 0) {
    image->transferCharacteristics =
        (avifTransferCharacteristics)options.cicpTransferCharacteristics;
  }
  if (options.cicpMatrixCoefficients >= 0) {
    image->matrixCoefficients = (avifMatrixCoefficients)options.cicpMatrixCoefficients;
  }
  image->yuvRange = options.fullRange ? AVIF_RANGE_FULL : AVIF_RANGE_LIMITED;
...
EMSCRIPTEN_BINDINGS(my_module) {
  value_object<AvifOptions>("AvifOptions")
      ...
      .field("cicpColorPrimaries", &AvifOptions::cicpColorPrimaries)
      .field("cicpTransferCharacteristics", &AvifOptions::cicpTransferCharacteristics)
      .field("cicpMatrixCoefficients", &AvifOptions::cicpMatrixCoefficients)
      .field("fullRange", &AvifOptions::fullRange);
